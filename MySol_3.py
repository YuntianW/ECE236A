...
def test_classify_class(data_x, data_y, solution):
    ...
    return result, acc
epsilon=np.linspace(-1,1,201)
for i in range(0,len(epsilon)):
    solution = train_all_classes(data, epsilon[i])
    predict, accuracy = test_classify_class(testX, testY, solution)
    print(epsilon[i],accuracy)
#
...
# color=['r','b','g']
# default_color=['#1f77b4', '#ff7f0e', '#2ca02c']
# for i in range(predict.shape[1]):
#     plt.scatter(testX[i,0],testX[i,1],color=default_color[int(testY[i])])
#
# for i in range(3):
#     y=-solution[0][i,0]/solution[0][i,1]*x+(1-solution[0][i,2])/solution[0][i,1]
#     plt.plot(x,y,color[i])
# plt.legend(['1','2','3'])
# plt.axis((-6,6,-6,6))
# plt.title('Ground Truth and Separation Boundary')
# plt.subplot(1,2,2)
#
# for i in range(predict.shape[1]):
#     plt.scatter(testX[i,0],testX[i,1],color=default_color[int(predict[0,i])])
# for i in range(3):
...
# plt.legend(['1','2','3'])
# plt.axis((-6,6,-6,6))
# plt.title('Classification Result and Separation Boundary\nAccuracy: '+str(accuracy))
# fig2.savefig('fig2.png',bbox_inches='tight',dpi=600,pad_inches = 0.05)
# plt.show()
